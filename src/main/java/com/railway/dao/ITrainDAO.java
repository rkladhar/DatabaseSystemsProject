...
//    List<TrainResult>getTrains(Long departureStationId, Long arrivalStationId, LocalDate dateOfJourney);
    ...
    @Query(value = "SELECT t.*, ta.available_seats, ta.date_of_journey, ta.ticket_price from train t join trainadditionalinfo ta on t.train_no = ta.train_no" +
            "            where t.arrival_station_id=:arrivalStationId And t.departure_station_id=:departureStationId" +
            "            And ta.date_of_journey = :dateOfJourney", nativeQuery = true)
    List<TrainResult>getTrains(Long departureStationId, Long arrivalStationId, LocalDate dateOfJourney);
}
